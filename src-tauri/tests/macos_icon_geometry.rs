//! Guards the MaxCode artwork and safe-area inset baked into `icons/icon.icns`.
//!
//! macOS draws `CFBundleIconFile` into a fixed tile, so the *artwork* — not the
//! canvas — decides how big the app reads in the Dock. The custom MaxCode icon
//! includes its own transparent breathing room around the layered terminal
//! artwork, so its visible body is slightly smaller than Apple's nominal
//! 824/1024 grid while still matching neighbouring app icons.
//!
...
//!
//! Why a test and not a comment: generic icon regeneration can silently replace
//! the fork's layered-terminal artwork with an unrelated source SVG. A red CI
//! cell makes that branding regression visible.

use std::collections::BTreeSet;
use std::path::PathBuf;

/// The custom artwork's visible body is about 0.77x0.78 of its canvas. Keep a
/// narrow tolerance for small-slot resampling while rejecting both full-bleed
/// icons and the unrelated upstream artwork.
const MIN_RATIO: f64 = 0.75;
const MAX_RATIO: f64 = 0.80;

/// The 1024 master is the slot the Dock actually scales from. The custom white
/// plate starts roughly 110px from each edge, including its own internal inset.
const MASTER_CANVAS: u32 = 1024;
const MASTER_MIN_INSET: u32 = 100;
const MASTER_MAX_INSET: u32 = 125;
    ...
    for (kind, expected_canvas) in PNG_SLOTS {
        ...
        assert!(
            ...
            "{kind}: body is {:.1}px of a {w}px canvas (ratio {ratio:.3}), outside \
             {MIN_RATIO}..={MAX_RATIO}. Restore the branded MaxCode icon.",
            body.width
        );
    }
}
...
#[test]
fn master_slot_keeps_the_maxcode_safe_area() {
    let data = std::fs::read(icns_path()).expect("read icons/icon.icns");
    let chunks = parse_chunks(&data);
    let payload = chunks
        .iter()
    ...
    for (axis, measured) in [("width", body.width), ("height", body.height)] {
        let ratio = measured / f64::from(MASTER_CANVAS);
        assert!(
            (MIN_RATIO..=MAX_RATIO).contains(&ratio),
            "1024 master {axis} is {measured:.1}px (ratio {ratio:.3}), outside \
             {MIN_RATIO}..={MAX_RATIO}"
        );
    }
    for (edge, measured) in [
        ...
        assert!(
            (MASTER_MIN_INSET..=MASTER_MAX_INSET).contains(&measured),
            "1024 master {edge} inset is {measured}px, outside \
             {MASTER_MIN_INSET}..={MASTER_MAX_INSET}"
        );
    }
}
...
fn icns_keeps_the_legacy_masks_the_declared_macos_floor_needs() {
    ...
    for kind in LEGACY_SLOTS {
        assert!(
            ...
            "icon.icns lost the {kind} chunk required by the declared macOS floor."
        );
    }
