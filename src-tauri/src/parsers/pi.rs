...
fn provider_declared_context_window(config: &Value, model: &str) -> Option<u64> {
    ...
    if let Some(window) = overridden {
        // An explicit invalid override must not silently revive a larger base
        // declaration that Pi would not be running with.
        return window.as_u64().filter(|window| *window > 0);
    }
...
#[cfg(test)]
#[path = "../../../src/maxcode-contracts/pi-model-window.contract.rs"]
mod maxcode_model_window_contract;
