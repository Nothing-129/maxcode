...
mod tests {
    ...
    fn maxcode_semantic_mcp_preserves_billing_timing_and_context_filtering() {
        let mut lines = code_mode_rollout(
            "const r=await tools.mcp__demo__run({cmd});text(r);",
            serde_json::json!("Script completed\nWall time 1.0 seconds\nOutput:\nfinished"),
        );
        lines.splice(0..0, [
            rollout_line("2026-07-20T08:39:57Z", "session_meta", serde_json::json!({
                "id": "maxcode-semantic", "cwd": "/tmp/demo",
            })),
            rollout_line("2026-07-20T08:39:58Z", "turn_context", serde_json::json!({
                "model": "gpt-model-a",
            })),
            rollout_line("2026-07-20T08:39:59Z", "response_item", serde_json::json!({
        ...
        lines.extend([
            rollout_line(
                ...
                "event_msg",
                serde_json::json!({
                    "type": "agent_message", "message": "answer one",
                }),
            ),
            rollout_line(
                ...
                serde_json::json!({
                    "type": "token_count", "info": {"total_token_usage": {
                        "input_tokens": 100, "cached_input_tokens": 10, "output_tokens": 10,
                    }},
                }),
            ),
            rollout_line(
            ...
            rollout_line(
                "2026-07-20T08:40:07Z",
                "event_msg",
                serde_json::json!({
                    "type": "agent_message", "message": "answer two",
                }),
            ),
            rollout_line(
                "2026-07-20T08:40:08Z",
                "event_msg",
                serde_json::json!({
                    "type": "token_count", "info": {"total_token_usage": {
                        "input_tokens": 300, "cached_input_tokens": 30, "output_tokens": 30,
                    }},
                }),
            ),
        ]);
        let baseline = parse_lines(&lines, "maxcode-semantic-baseline");
        lines.insert(5, rollout_line("2026-07-20T08:40:01Z", "event_msg", serde_json::json!({
            "type": "item_completed", "item": {
                "type": "McpToolCall", "id": "semantic-call", "server": "demo", "tool": "run",
                "arguments": {"cmd": "pnpm build"}, "status": "completed",
                "result": {"content": [{"type": "text", "text": "finished"}], "isError": false},
            },
        })));
        let recovered = parse_lines(&lines, "maxcode-semantic-recovered");
        assert_eq!(tool_uses(&recovered)[0].0, "semantic-call");
        let metadata =
            |detail: &crate::models::ConversationDetail| {
                detail.turns.iter().map(|turn| serde_json::json!({
                "model": turn.model, "usage": turn.usage, "duration_ms": turn.duration_ms,
                "role": turn.role,
            })).collect::<Vec<_>>()
            };
        assert_eq!(
            metadata(&recovered),
            metadata(&baseline),
            "tool recovery must not change turn accounting"
        );
        let billed: Vec<_> = recovered
            .turns
            .iter()
        ...
        assert!(recovered
            .turns
            .iter()
            .any(|turn| turn.duration_ms.is_some()));
        assert_eq!(recovered.summary.title.as_deref(), Some("go"));
        assert!(recovered.turns.iter().flat_map(|turn| &turn.blocks).all(|block| {
            !matches!(block, ContentBlock::Text { text } if text.contains("recommended_plugins"))
        }));
    }
    ...
    fn maxcode_semantic_mcp_keeps_existing_native_spawn_identity() {
        ...
        lines.splice(2..2, [
            rollout_line("2026-07-20T08:40:01Z", "response_item", serde_json::json!({
                "type": "function_call", "call_id": "native-spawn", "name": "spawn_agent",
                "namespace": "collaboration", "arguments": serde_json::json!({
                    "task_name": "build", "message": "build the project",
                }).to_string(),
            })),
            rollout_line("2026-07-20T08:40:01Z", "event_msg", serde_json::json!({
                "type": "sub_agent_activity", "event_id": "native-spawn",
                "agent_thread_id": "native-child", "agent_path": "/root/build", "kind": "started",
            })),
            rollout_line("2026-07-20T08:40:01Z", "response_item", serde_json::json!({
                "type": "function_call_output", "call_id": "native-spawn",
                "output": serde_json::json!({"task_name": "/root/build"}).to_string(),
            })),
        ]);
        let baseline = parse_lines(&lines, "maxcode-native-baseline");
        lines.insert(3, rollout_line("2026-07-20T08:40:01Z", "event_msg", serde_json::json!({
            "type": "item_completed", "item": {
                "type": "McpToolCall", "id": "semantic-call", "server": "demo", "tool": "run",
                "arguments": {"cmd": "pnpm build"}, "status": "completed",
                "result": {"content": [{"type": "text", "text": "finished"}], "isError": false},
        ...
        let recovered = parse_lines(&lines, "maxcode-native-recovered");
        let native = |detail: &crate::models::ConversationDetail| {
            tool_uses(detail)
                .into_iter()
                .find(|(id, _, _)| id == "native-spawn")
                .unwrap()
        };
        assert_eq!(native(&recovered), native(&baseline));
        let (_, name, input) = native(&recovered);
        assert_eq!(name, "Agent");
        let input: serde_json::Value = serde_json::from_str(input.as_deref().unwrap()).unwrap();
        assert_eq!(input["agent_id"], "native-child");
        assert_eq!(input[CODEX_SUBAGENT_LAUNCH_KEY], true);
        assert!(tool_uses(&recovered)
            .iter()
            .any(|(id, name, _)| id == "semantic-call" && name == "mcp__demo__run"));
    }
