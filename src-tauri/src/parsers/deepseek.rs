...
///
/// MaxCode preserves generation-first recovery, including a migration from
/// compressed legacy logs to newer plaintext logs. At the same generation,
/// Zstandard wins; never silently fall back if the selected log is corrupt.
    ...
    I: IntoIterator<Item = std::io::Result<(OsString, PathBuf)>>,
{
    ...
    match (best_zstd, best_raw) {
        (Some((zstd_version, zstd)), Some((raw_version, raw))) => {
            if raw_version > zstd_version {
                Some((raw, LogEncoding::Raw))
            } else {
...
mod tests {
    ...
    #[test]
    fn a_mixed_encoding_root_keeps_maxcode_generation_precedence() {
        ...
        assert_eq!(
            resolve_generation_log_path(&session_dir),
            Some((session_dir.join("session.v3.jsonl"), LogEncoding::Raw))
        );
        let parser = DeepSeekParser::with_base_dir(dir.clone());
        let detail = parser.get_conversation(GEN_ID).expect("detail");
        assert_eq!(user_texts(&detail), vec!["raw".to_string()]);
