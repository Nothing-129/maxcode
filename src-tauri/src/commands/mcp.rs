...
// into this file. The writer preserves pi-only extension fields that are not
// present in that winning spec, while replacing canonical transport fields.
// This keeps editing a shared server from erasing the extension's own options
// or leaking those options into every other agent's configuration.
//
...
fn upsert_pi_server_at(path: &Path, id: &str, spec: &Value) -> Result<(), AppCommandError> {
    ...
    let mut canonical = canonicalize_spec(spec, "pi write")?;
    ...
    // A shared server's editor spec may come from an earlier scan source
    // (e.g. Claude Code), which knows nothing about pi extension options.
    // Preserve those local fields unless the edit explicitly supplies a new
    // value. Transport fields remain authoritative: switching stdio/HTTP must
    // remove the old command/args/env/cwd or url/headers, not revive them.
    const TRANSPORT_KEYS: &[&str] = &["type", "command", "args", "env", "cwd", "url", "headers"];
    if let (Some(existing), Some(next)) = (
        map.get(id).and_then(Value::as_object),
        canonical.as_object_mut(),
    ) {
...
mod tests {
    ...
    fn pi_shared_server_edits_keep_extension_fields_but_replace_transport() {
        ...
        temp_env::with_var("PI_CODING_AGENT_DIR", Some(dir.path()), || {
            ...
            let mut shared = scan.servers.into_iter().find(|s| s.id == "shared").unwrap();
            assert_eq!(shared.apps, [McpAppType::ClaudeCode, McpAppType::Pi]);
            assert_eq!(shared.spec["command"], "claude-wins");
            assert!(shared.spec.get("extensionOption").is_none());
            shared.spec["command"] = json!("edited-shared");
            shared.spec["args"] = json!(["--shared"]);
            shared.spec["env"] = json!({"SHARED_KEY": "fixture"});
            shared.spec["cwd"] = json!("/shared/workspace");
            upsert_server_for_app(McpAppType::Pi, "shared", &shared.spec).unwrap();
            let edited = read_json_file(&path).unwrap();
            assert_eq!(edited["mcpServers"]["shared"]["command"], "edited-shared");
            assert_eq!(edited["mcpServers"]["shared"]["args"], json!(["--shared"]));
            assert_eq!(
                edited["mcpServers"]["shared"]["env"],
                json!({"SHARED_KEY": "fixture"})
            );
            assert_eq!(edited["mcpServers"]["shared"]["cwd"], "/shared/workspace");
            assert_eq!(
                edited["mcpServers"]["shared"]["extensionOption"],
                original["mcpServers"]["shared"]["extensionOption"]
            );
            assert_eq!(edited["mcpServers"]["shared"]["timeoutMs"], 3000);

            upsert_server_for_app(
                McpAppType::Pi,
                "shared",
                &json!({
                    "type": "http",
                    ...
                    "extensionOption": {"enabled": false}
                }),
            )
            .unwrap();
            let remote = read_json_file(&path).unwrap();
            let spec = &remote["mcpServers"]["shared"];
            for stale in ["command", "args", "env", "cwd"] {
                assert!(
                    spec.get(stale).is_none(),
                    "old stdio field survived: {stale}"
                );
            }
            assert_eq!(spec["type"], "http");
            assert_eq!(spec["extensionOption"], json!({"enabled": false}));
            assert_eq!(spec["timeoutMs"], 3000);
            assert_eq!(remote["settings"], original["settings"]);
            assert_eq!(
                remote["mcpServers"]["unrelated"],
                original["mcpServers"]["unrelated"]
            );

            upsert_server_for_app(McpAppType::Pi, "shared", &json!({"command": "stdio-again"}))
                .unwrap();
            let local = read_json_file(&path).unwrap();
            let spec = &local["mcpServers"]["shared"];
            assert!(spec.get("url").is_none());
            assert!(spec.get("headers").is_none());
