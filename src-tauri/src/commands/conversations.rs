...
fn apply_in_flight_message_id(
    ...
    //
    // `gate_fired` is false for both a conversation's first turn and a parser
    // that substituted parse instants. Multiple matching user turns are then
    // ambiguous. Even a SINGLE match may be from a completed earlier round
    // while an identical new prompt has not yet been persisted: if a completed
    // assistant follows it, do not stamp it. A live partial response without a
    // completion marker remains eligible, preserving first-turn streaming and
    // mid-turn steering. Refusing only produces a recoverable visible duplicate;
    // mis-stamping an old prompt can hide the genuinely new one.
    //
    ...
    if !gate_fired
        && (matched > 1
            || turns[target_idx + 1..].iter().any(|turn| {
...
mod tests {
    ...
    fn stamps_user_turn_before_partial_trailing_assistant_regardless_of_completion() {
        ...
        let mut turns = vec![
            user_text_turn("old-user", "earlier", at(-30)),
            assistant_text_turn("old-reply", "done", at(-29), true),
        ...
        assert_eq!(turns[2].id, "msg-live");
        assert_eq!(turns.len(), 4, "the partial reply is preserved");
        assert_eq!(turns[3].id, "turn-1", "the partial reply is untouched");
    }
    ...
    #[test]
    fn parse_time_does_not_make_an_old_completed_round_look_in_flight() {
        // The new "continue" has not reached disk yet. A parser that replaced
        // missing record times with this fetch's time makes the old prompt pass
        // the recency gate, and it is the only content match. The completed
        // reply after it still proves this is unsafe to stamp.
        let parsed_at = at(1);
        let mut turns = vec![
            user_text_turn("turn-0", "continue", parsed_at),
            assistant_text_turn("turn-1", "done", parsed_at, true),
    ...
    fn stamps_an_unambiguous_match_even_with_no_proof_of_where_the_turn_began() {
        // Same parse-instant transcript, but the prompt's text occurs once
        // and the partial reply has no completion marker. This remains eligible
        // even with a later mid-turn user message.
        //
    ...
    async fn batch_import_nests_a_worktree_under_a_repo_created_in_the_same_run() {
        ...
        let temp = tempfile::tempdir().expect("tempdir");
        let root = std::fs::canonicalize(temp.path()).expect("canonicalize root");
        let (original_repo, worktree) = crate::git_repo::fixture_linked_worktree(&root);
        // The worktree MUST sort before its main repo; otherwise the original
        ...
        .expect("update worktree pointer");
    ...
    // A flattened candidate must still exist and be live: folder.parent_id
    // has no foreign-key constraint and can contain stale references.
    #[tokio::test]
    async fn batch_import_refuses_dangling_or_deleted_flattened_roots() {
        use sea_orm::EntityTrait;
        for deleted_root in [false, true] {
            let db = fresh_in_memory_db().await;
            let temp = tempfile::tempdir().expect("tempdir");
            let root = std::fs::canonicalize(temp.path()).expect("canonicalize root");
            let (repo, worktree) = crate::git_repo::fixture_linked_worktree(&root);
            let parent_id = if deleted_root {
                let id = seed_folder(&db, &root.join("outer").to_string_lossy()).await;
                folder_service::soft_delete_folder(&db.conn, id)
                    .await
                    .expect("delete root");
                id
            } else {
            ...
            folder_service::add_folder_with_parent(
                &db.conn,
                &repo.to_string_lossy(),
                Some(parent_id),
            )
            .await
            .expect("seed stale parent");
            ...
            assert_eq!(result.imported, 1);
            let row = crate::db::entities::folder::Entity::find_by_id(result.folders[0].folder_id)
                .one(&db.conn)
                .await
                .unwrap()
                .unwrap();
            assert_eq!(
                row.parent_id, None,
                "unsafe root must leave the import visible"
            );
        }
    }
