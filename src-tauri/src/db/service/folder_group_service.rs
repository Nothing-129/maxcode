...
mod tests {
    ...
    #[tokio::test]
    async fn a_newly_opened_folder_keeps_maxcodes_highest_priority() {
        let db = fresh_in_memory_db().await;
        let conn = &db.conn;
        // MaxCode puts a newly opened folder at the front of the sidebar. This
        // remains true when upstream folder groups already occupy the shared
        // top-level sequence.
        ...
        assert_eq!(
            ...
            0,
            "new folders retain MaxCode's highest-priority position"
        );
    }
