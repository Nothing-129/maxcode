...
async fn start_claimed(
    ...
    let signal = tokio::time::timeout(URL_WAIT, await_start_signal(&mut url_rx, &mut responses))
        .await
...
async fn spawn_agent(
    ...
    // The authorization URL moved from stdout to stderr in Antigravity 1.1.1,
    // so both pumps share the same first-wins sink.
...
/// Wait for either the sign-in URL or the authenticate response. Closing
/// stdout only ends JSON-RPC responses: stderr can still have an unread URL.
/// The shared URL channel closes after both output pumps have finished.
async fn await_start_signal(
    url_rx: &mut oneshot::Receiver<String>,
    responses: &mut mpsc::UnboundedReceiver<serde_json::Value>,
) -> StartSignal {
    ...
    loop {
        tokio::select! {
            ...
            message = responses.recv(), if responses_open => {
                let Some(message) = message else {
                    // Disable the EOF branch; polling a closed receiver again
                    // would spin while stderr is still delivering its output.
...
mod tests {
    ...
    async fn stdout_eof_does_not_discard_a_later_stderr_sign_in_link() {
        ...
        const URL: &str = "https://accounts.google.com/o/oauth2/v2/auth?state=delayed";
        let (sink, mut url_rx) = AuthUrlSink::new();
        let (tx, mut responses) = mpsc::unbounded_channel();
        read_agent_stdout(Cursor::new(Vec::<u8>::new()), tx, sink.clone()).await;

        let mut waiting = Box::pin(await_start_signal(&mut url_rx, &mut responses));
        // Poll the real selection loop after stdout closes but before stderr
        // delivers its line. Previously it returned Gone right here.
        ...
        let prompt = format!("{AUTH_PROMPT_MARKER}{URL}\n");
        read_agent_stderr(
            Cursor::new(prompt.into_bytes()),
            Arc::new(StderrTail::new()),
            sink,
        )
        .await;
        match waiting.await {
            StartSignal::Url(url) => assert_eq!(url, URL),
            _ => panic!("stderr URL must win after stdout EOF"),
        }
    }

    #[tokio::test]
    async fn start_signal_is_gone_after_both_output_pumps_close() {
        let (sink, mut url_rx) = AuthUrlSink::new();
        let (tx, mut responses) = mpsc::unbounded_channel();
        read_agent_stdout(Cursor::new(Vec::<u8>::new()), tx, sink.clone()).await;
        read_agent_stderr(
        ...
        .await;
        assert!(matches!(
            await_start_signal(&mut url_rx, &mut responses).await,
            StartSignal::Gone
        ));
    }
    ...
    async fn start_signal_retains_cached_authentication_and_error_results() {
        for rejected in [false, true] {
            let (sink, mut url_rx) = AuthUrlSink::new();
            let (tx, mut responses) = mpsc::unbounded_channel();
            tx.send(serde_json::json!({"id": ID_INITIALIZE, "result": {}}))
                .unwrap();
            tx.send(if rejected {
                serde_json::json!({
                    "id": ID_AUTHENTICATE,
                    "error": {"message": "method refused"}
                })
            } else {
                serde_json::json!({"id": ID_AUTHENTICATE, "result": {}})
            })
            .unwrap();
            match await_start_signal(&mut url_rx, &mut responses).await {
                StartSignal::Failed(reason) if rejected => assert_eq!(reason, "method refused"),
                StartSignal::Authenticated if !rejected => {}
                _ => panic!("authenticate response must resolve without waiting for a URL"),
            }
            drop(sink);
        }
    }
