...
describe("SidebarConversationList — Recent section", () => {
  ...
  describe("paging", () => {
    ...
    // Recent duplicates canonical rows, but the merged MaxCode sidebar also
    // pages each folder independently. Subtract only the canonical rows that
    // are actually visible so these assertions isolate the Recent slice.
    ...
    it("offers the reset at the row's right edge while pages remain", () => {
      ...
      const recentRows = () =>
        Array.from(document.querySelectorAll("[data-conversation-id]")).length -
        Math.min(conversations.length, FOLDER_PAGE)
...
describe("SidebarConversationList — expand / collapse all", () => {
  ...
  it("re-opens every section and the folder groups under them", () => {
    const ref = renderList()
    act(() => ref.current?.collapseAll())
...
describe("SidebarConversationList — folder groups", () => {
  ...
  it("keeps MaxCode folder theming scoped to the folder row wrapper", () => {
    groupWith([5], [conv(11, 5)])
    const { container } = render(tree())

    expect(container.querySelector("[data-conversation-id]")).not.toBeNull()
    expect(container.querySelectorAll('[data-theme="blue"]')).toHaveLength(1)
  })
