...
function canvasAgentMemoryScope(
  target: NewConversationTarget,
  folders: readonly FolderDetail[]
): string {
  if ("chat" in target) return CHAT_AGENT_MEMORY_SCOPE
  return (
    folders.find((folder) => folder.id === target.folderId)?.path ??
...
function CanvasFlow() {
  ...
  const setDraftAgent = useCallback(
    (draftId: string, agentType: AgentType) => {
      ...
      if (draft) {
        saveLastSelectedAgent(
          canvasAgentMemoryScope(draft.target, allFolders),
          agentType
        )
      }
    ...
    (point: { x: number; y: number }) => {
      ...
      const { agentType } = resolveDefaultAgent({
        lastSelected: getLastSelectedAgent(
          canvasAgentMemoryScope(target, allFolders)
        ),
        folderDefault,
        inherit: null,
        sortedTypes: [],
