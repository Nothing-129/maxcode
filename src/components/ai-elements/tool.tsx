...
import {
  ActivityStatusIcon,
  type ActivityStatus,
} from "@/components/shared/activity-status-icon"
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
...
const activityStates: Record<ToolPart["state"], ActivityStatus> = {
...
export const getStatusBadge = (status: ToolPart["state"], label: string) => (
  <Badge className="gap-1.5 rounded-full text-xs" variant="secondary">
    <ActivityStatusIcon status={activityStates[status]} className="size-4" />
    {label}
  </Badge>
)
