...
export function rememberModelLabels(
  ...
  for (const option of selectOptions) {
    ...
    // An explicit rename back to the raw id must clear a stale friendly label.
    // Missing catalog entries still retain their history labels.
    ...
    if (current?.get(option.value) === label) continue
    next ??= new Map(existing)
