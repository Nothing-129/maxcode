...
describe("MaxCode agent npm registry", () => {
  it("keeps manual installs on the mirror but checks official published releases", () => {
    ...
    expect(updates).toContain("reqwest::Url::parse(OFFICIAL_NPM_REGISTRY)")
    ...
    for (const code of [install, custom]) {
